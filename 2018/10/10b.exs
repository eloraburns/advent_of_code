defmodule Ten do
  ...
  def minimize_by(d, f, by) do
    minimize_by(d, by.(d), f, by, 0)
  end

  defp minimize_by(d, v, f, by, seconds) do
    d2 = f.(d)
    v2 = by.(d2)
    if v2 >= v do
      {d, seconds}
    else
      minimize_by(d2, v2, f, by, seconds + 1)
    end
  end

  def display({points, seconds}) do
    {minx, maxx, miny, maxy} = points |> bounds
    ...
    |> IO.puts
    IO.puts "Shows in #{seconds} seconds"
  end
