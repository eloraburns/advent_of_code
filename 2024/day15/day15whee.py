...
class Warehouse:
    ...
    def parse(self, raw_map):
        ...
        for (y, l) in enumerate(raw_map.split("\n")):
            for (x, c) in enumerate(l):
                if c == '#':
                    m.append(wall := Wall(x*2, y))
                elif c == 'O':
                    m.append(box := Box(x*2, y))
                elif c == '@':
    ...
    def clear(self, coords):
        for coord in coords:
            del self.occupant_map[coord]

    def set(self, other):
        for coord in other.occupies():
            self.occupant_map[coord] = other

    def find_occupant(self, x, y):
        if self.robot.does_occupy(x, y):
            return self.robot
        return self.occupant_map.get((x, y), Empty())
    ...
    def __str__(self):
        out = []
        maxx = max(x for (x, y) in self.occupant_map.keys())
        #maxx = 0
        #for e in self.map:
